...
export default function AnnextureForm() {
  ...
  // Load existing annexture if editing
  useEffect(() => {
    if (isNew) {
      setFormData((prev) => ({
        ...prev,
      ...
      const existingAnnexture = annextures.find((a) => a.workOrderId === workOrderId);
      if (existingAnnexture) {
        setFormData({
          workOrderId: existingAnnexture.workOrderId,
          quotationId: existingAnnexture.quotationId,
          clientName: existingAnnexture.clientName,
          projectName: existingAnnexture.projectName,
          createdDate: existingAnnexture.createdDate,
          preparedBy: existingAnnexture.preparedBy,
          approvedBy: existingAnnexture.approvedBy,
          approvalStatus: existingAnnexture.approvalStatus,
          items: existingAnnexture.items.map((item) => ({
            ...item,
            id: item.id || `item-${Date.now()}`,
          })),
          accessories: existingAnnexture.accessories.map((acc) => ({
            ...acc,
            id: acc.id || `acc-${Date.now()}`,
          })),
          notes: existingAnnexture.notes,
          attachments: existingAnnexture.attachments || [],
        });
      }
    }
  }, [workOrderId, isNew]);
          ...
          <Col md="12">
            <Card className="mb-4">
              <Card.Header style={{ backgroundColor: "#2c3e50" }}>
                <Card.Title as="h4">
                  {isNew ? "Create Annexture" : "Edit Annexture"}
                </Card.Title>
              </Card.Header>
              <Card.Body>
                <Row>
