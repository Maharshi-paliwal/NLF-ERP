...
const PDFPreview = ({ show, onHide, quotationData }) => {
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            const fileName = `Quotation_${quotationData?.quotationId || 'Unknown'}_${quotationData?.currentRound?.round || 'Latest'}.pdf`;
            pdf.save(fileName);
        } catch (error) {
    ...
    const getCurrentRoundData = () => {
        ...
        if (quotationData.rounds && quotationData.rounds.length > 0) {
            if (quotationData.currentRound) {
                const specificRound = quotationData.rounds.find(r => r.round === quotationData.currentRound);
                if (specificRound) return specificRound;
            }
            const sortedRounds = [...quotationData.rounds].sort(
    ...
    // ===== FIXED: GET OFFICE BRANCH FROM QUOTATION DATA =====
    // Priority: quotationData.officeBranch (from top level) -> default to 'Kolkata'
    const officeBranch = quotationData?.officeBranch || 'Kolkata';

    console.log('DEBUG - Quotation Data:', quotationData); 
    ...
    const getHeaderImagePath = (branch) => {
        ...
        // Return matching branch or fallback to Kolkata
    ...
    return (
        <Modal show={show} onHide={onHide} size="lg" centered>
            <Modal.Header closeButton>
                <Modal.Title>Quotation Preview - {quotationData.quotationId}</Modal.Title>
            </Modal.Header>
            <Modal.Body style={{ maxHeight: '80vh', overflowY: 'auto' }}>
                ...
                >
                    ...
                    <div style={{ border: '2px solid #000', margin: '10px' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 10px', borderBottom: '1px solid #000', fontSize: '11px' }}>
                            <span style={{ fontWeight: 'bold' }}>
                                ...
                                {currentRound?.round && currentRound.round !== '' && `-${currentRound.round}`}
                            </span>
                            <span>Date - {currentRound?.date || new Date().toLocaleDateString('en-IN')}</span>
                        </div>
                        ...
                        <div style={{ padding: '6px 10px', borderBottom: '1px solid #000', fontSize: '11px' }}>
                            ...
                            <div style={{ marginBottom: '2px', fontWeight: 'bold' }}>
                                {quotationData.customer?.name || 'Customer Name'}
                            </div>
                            <div style={{ marginBottom: '2px' }}>
                                {quotationData.customer?.city || 'City'}
                            </div>
                        </div>
                        ...
                        <div style={{ padding: '6px 10px', borderBottom: '1px solid #000', fontSize: '11px' }}>
                            <div style={{ marginBottom: '3px' }}>
                                <strong>Kind Attention: {quotationData.salespersonName || 'Sales Team'}</strong>
                            </div>
                            <div style={{ marginBottom: '3px' }}>
                                <strong>Subject:- Quotation for {currentRound?.details || 'Professional Services'}</strong>
                            </div>
                        </div>
                                ...
                                {currentRound?.items?.length > 0 ? (
                                        ...
                                        <tr key={index}>
                                            ...
                                            <td style={{ padding: '8px', border: '1px solid #000', verticalAlign: 'top', fontSize: '9px', lineHeight: '1.3' }}>
                                                <div style={{ fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '3px' }}>
                                                    {item.material ? `${item.material.toUpperCase()} FOR RAIN PROTECTION:` : 'PROFESSIONAL SERVICE:'}
                                                </div>
                                                <div>
                                                    {item.description}
                                                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td style={{ padding: '8px', border: '1px solid #000', verticalAlign: 'top', fontSize: '9px', lineHeight: '1.3' }}>
                                            ...
                                            <div>
                                                {currentRound?.details || `Complete professional service solution for ${quotationData.quotationId}. All work to be completed as per specifications and requirements with quality materials and professional installation.`}
                                            </div>
                                        </td>
                                        <td style={{ padding: '8px', border: '1px solid #000', textAlign: 'center', verticalAlign: 'top' }}>Job</td>
                                        <td style={{ padding: '8px', border: '1px solid #000', textAlign: 'center', verticalAlign: 'top' }}>1</td>
                                        <td style={{ padding: '8px', border: '1px solid #000', textAlign: 'right', verticalAlign: 'top' }}>
                                            {(currentRound?.amount || quotationData.amount || 0).toLocaleString('en-IN')}
                                        </td>
                                        <td style={{ padding: '8px', border: '1px solid #000', textAlign: 'right', verticalAlign: 'top' }}>
                                            {(currentRound?.amount || quotationData.amount || 0).toLocaleString('en-IN')}
                                        </td>
                                    ...
                                    <tbody>
                                        <tr style={{ backgroundColor: '#4A90E2', color: 'white' }}>
                                            ...
                                            <td style={{ padding: '6px', border: '1px solid #000', textAlign: 'right', fontWeight: 'bold' }}>
                                                {(() => {
                                                    const total = currentRound?.amount || quotationData.amount || 0;
                    ...
                    <div style={{
                        ...
                        <img src="/extra/pdfpreviewFooter.jpeg" style={{width:"775px", height:"75px"}} alt="Footer Banner" />
                    </div>
                </div>
            </Modal.Body>
            <Modal.Footer>
